...
import jp.redmine.gttnl.fragment.form.IssueFilterHeaderForm;
...
import jp.redmine.gttnl.task.SelectIssueTask;
import jp.redmine.gttnl.task.SelectProjectEnumerationTask;
...
public class IssueList extends OrmLiteListFragment<DatabaseCacheHelper> implements SwipeRefreshLayout.OnRefreshListener {
	...
	public boolean onOptionsItemSelected(MenuItem item)
	{
		...
		} else if (item.getItemId() == R.id.menu_issues_filter) {
			intentFilterAction();
			return true;

		} else if (item.getItemId() == R.id.menu_access_addnew) {
			ProjectArgument intent = new ProjectArgument();
			intent.setArgument(getArguments());
			mListener.onIssueAdd(intent.getConnectionId(), intent.getProjectId());
			return true;
